...
async function main() {
  ...
  // 4. Match Score Cached (POST with jobIds)
  try {
    const { status, body } = await api("/api/match/score/cached", {
      method: "POST",
      body: JSON.stringify({ jobIds: ["cmoj74crq00zapf01hn1dv7fa"] }),
    });
  ...
  // 6. Email Repair Drafts (admin-only endpoint — 401 is expected)
  try {
    const { status, body } = await api("/api/email/repair-drafts", {
      method: "POST",
      body: JSON.stringify({ dryRun: true }),
    });
    // 401/403 means endpoint is reachable but requires auth — that's OK
    const reachable = status !== 500;
    record(
      "Email Repair Drafts (reachable, admin-only)",
      reachable,
      `status=${status}, msg=${body?.error?.message || "ok"}`,
    );
  } catch (e) {
