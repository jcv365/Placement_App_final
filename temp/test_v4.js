...
async function main() {
  ...
  console.log(`Found ${apps.length} eligible remote non-US applications`);
  for (const app of apps) {
    console.log(
      `${app.id} | ${app.job.title} | ${app.candidate.fullName} | JD:${app.job.rawText?.length || "NONE"} | CV:${app.candidate.rawCV?.length || "NONE"}`,
    );
  }
