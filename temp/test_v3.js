...
async function main() {
  const apps = await p.application.findMany({
    where: {
      ...
      emails: { none: {} },
      job: { createdAt: { gte: new Date("2026-04-25") }, isRemote: true },
    },
    select: {
      id: true,
      jobId: true,
      candidateId: true,
      job: { select: { title: true, rawText: true, isRemote: true } },
      candidate: { select: { fullName: true, rawCV: true } },
    },
    take: 5,
  });

  console.log(`Found ${apps.length} eligible remote applications`);
  for (const app of apps) {
    ...
    console.log(
      `${app.id} | ${app.job.title} | ${app.candidate.fullName} | JD:${hasJD ? app.job.rawText.length : "NONE"} | CV:${hasCV ? app.candidate.rawCV.length : "NONE"} | remote:${app.job.isRemote}`,
    );
  }
