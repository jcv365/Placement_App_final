...
async function main() {
  // Find a SHORTLISTED app without drafts, job after 25 April, remote=true, no US auth
  ...
  if (!app) {
    console.log("No eligible app found with isRemote=true and usAuth=null after 25 April");
    await p.$disconnect();
    return;
  }
  console.log("Testing:", app.id, app.job.title, app.candidate.fullName, "jobCreated:", app.job.createdAt.toISOString());
